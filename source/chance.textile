...
* Import the Compass framework, extending SCSS' usability.
...
In addition to having its own CSS extensions, Chance includes "*_SCSS_*":http://sass-lang.com/ and "*_Compass_*":http://compass-style.org. Even with all of these extensions, Chance is a superset of CSS. This means that 
...
Chance can also include the "Compass":http://compass-style.org framework; a set of reusable SCSS mixins, helpers and utilities that simplify CSS3 declarations using SCSS.

NOTE: Just like Sass, Compass' own build tools process files differently than Chance. Remember, SproutCore sends _all_ CSS, including +.css+ files, through SCSS and Compass *(if imported)*. Give your files the +.css+ extension.

Compass has many mixins, helpers and utilities. For full reference, refer to the official "Compass":http://compass-style.org documentation.

h4. +@import+ Directive

Compass can be included in any of your theme's +.css+ files by adding the +@import "compass/css3";+ directive at the top of the file. This will tell SCSS to include the Compass framework.

In addition, you can include Compass globally by adding +@import "compass/css3";+ to your +_theme.css+ file.

<css filename="apps/my_app/resources/_theme.css">
@import "compass/css3";

$theme: '.ace.MyTheme';
</css>

NOTE: Any +_theme.css+ file is prepended to all files inside its directory, and any subdirectories that don't define their own +_theme.css+ file. This allows you to give different directories different values for +$theme+ if you wish.

h4. Mixins

Compass makes it easy to add-in CSS3 declations in a single-line, making it easy to create effects and removing the redundancy of supporting multiple browsers. The +background-image+ and +linear-gradient+ mixin are an example of how Compass utilizes SCSS to reduce the amount of lines necessary to draw a gradient on the document body.

<css filename="apps/my_app/resources/sample.css">
@import "compass/css3";

body {
  @include background-image(linear-gradient(left top,  #fefcea 0%,#f1da36 100%));
}
</css>

After processing, Compass will have produced the CSS necessary to show a gradient on all browsers that support CSS3 linear-gradient declaration:
...
* April    5, 2012: added the Compass section and some other minor updates by "Steven Jabour":credits.html#stevenjabour